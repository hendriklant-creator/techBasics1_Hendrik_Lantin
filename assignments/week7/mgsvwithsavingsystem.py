# Assignment 7 - added a record-saving system on top of my Assignment 6 game
# The game is still my Metal Gear Solid V prologue (my favourite game ever)
# NEW THIS WEEK: it now times your playthrough, saves your result to a CSV file,
# and shows a leaderboard of the fastest escapes at the end.
#
# I marked all the new week 7 stuff with "NEW" comments so it's easy to see what got added on top of last week's game.


# NEW
# csv and os we used in class. datetime i had to look up because the assignment needs a timestamp and i don't know how to do that. it just gives the date+time right now.
#
import csv
import os
...
#NEW
# debug flag - when this is True the game is skipped and only asks for a name,
# then saves a placeholder result. handy for testing the saving without playing.
DEBUG = True

# the file all the records get saved into
...
def ending(player_choice):
    ...
    # NEW this now returns the ending name so main() can save it!!!
    global scene_over
    print("")
    ...
    # i save the ending name in a variable so i can return it at the end
    ...
    print("\n END OF PROLOGUE")
    scene_over = True
    #NEW hand the ending back so it can be saved =====
    return ending_name


#NEW RECORD-SAVING FUNCTIONS

def save_record(name, ending, seconds):
    # saves one result line to the CSV file.
    # if the file doesn't exist yet, then it writes the header row (the column titles).
    # "a" mode appends, and it also creates the file if it's not there already (from class notes, but it did not get it completely so I used AI for this!!!).
    file_exists = os.path.exists(RECORDS_FILE)
    with open(RECORDS_FILE, "a", newline="") as f:
        writer = csv.writer(f)
        if not file_exists:
            # no file found yet, means it start it with a header row
            writer.writerow(["name", "timestamp", "ending", "seconds"])
        # datetime.now() gives the current date and time, strftime makes it a nice string?? I really don't get this and I did not have time this week to correct it and do it myself
        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M")
        writer.writerow([name, timestamp, ending, seconds])
    print("\n(Your result was saved to " + RECORDS_FILE + ")")


def show_leaderboard():
    # reads all the saved records back and prints them fastest-first.
    # try/except handles the case where the file somehow isn't there.
    try:
        rows = []
        with open(RECORDS_FILE) as f:
            reader = csv.DictReader(f)  # reads each line as a dictionary (got this from the stuff we did in class)
            for row in reader:
                rows.append(row)
    except FileNotFoundError:
        print("No records found yet, so no leaderboard to show.")
        return

    # sort the rows so the smallest time is first.
    # we didn't learn sorting dictionaries by a key in class, so i just compare
    # every pair and swap them if they're in the wrong order (smaller seconds = better).
    for i in range(len(rows)):
        for j in range(i + 1, len(rows)):
            if int(rows[j]["seconds"]) < int(rows[i]["seconds"]):
                rows[i], rows[j] = rows[j], rows[i]

    print("\nLEADERBOARD - fastest escapes")
    place = 1
    for row in rows:
        # the :12s and :>4 just line the columns up well. (the f-string padding from class and some kind of logic that i thought of : = )
        print(f"{place}. {row['name']:12s} {row['seconds']:>4}s   [{row['ending']}]   {row['timestamp']}")
        place += 1
    print("------------------------")
...
def main():
    ...
    #NEW ask the player's name first (needed for the record)
    print("  THE PHANTOM PAIN - Prologue (text version)")
    player_name = input("\nBefore we begin, what's your name, soldier? ").strip()
    if player_name == "":
        player_name = "Unknown"

    #NEW - debug mode - skip the whole game, just test the saving
    if DEBUG:
        print("\n[DEBUG MODE ON] Skipping the game and saving a placeholder result.")
        save_record(player_name, "debug", 0)
        show_leaderboard()
        return

    #NEW start a timer so i can measure how long the escape took
    ...
    # this will hold which ending the player got
    final_ending = "unknown"
    ...
    while not scene_over:
        ...
        if assassin_arrived:
            if action == "pickup" and target == "scalpel":
                final_ending = ending("scalpel")
            elif action == "pickup" and target == "pistol":
                final_ending = ending("pistol")
            elif action == "wait":
                final_ending = ending("wait")
            else:
        ...
        # automatic story triggers for the dialogue to kick in
    ...
    #NEW game finished - here it works out the time taken and save the record
    end_time = datetime.now()
    # subtracting two datetimes gives a duration, .seconds gets whole seconds out of it
    seconds_taken = (end_time - start_time).seconds
    save_record(player_name, final_ending, seconds_taken)
    show_leaderboard()
